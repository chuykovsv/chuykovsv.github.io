...
export class Map {
    ...
    testBlock(x, y, z) {
        ...
        let uh = 0;
        const genChunks = this.getGenChunks(cx, cz, cy);
        const unit = this.chunks[it].getUnit(ix, iz, iy, genChunks);

        uh = this.unitHeight(unit);
        if (uh === 0) {
            const unit1 = this.chunks[it].getUnit(ix, iz, iy - 1, genChunks);
            const hh = this.unitHeight(unit1);
            return hh + iy - 1 + cy * Chunk.SIZE;
        }

        if (uh === 0.5) {
            return iy + uh + cy * Chunk.SIZE;
        }
